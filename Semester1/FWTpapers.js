...
class FWTpapers extends Component {
  static navigationOptions = {
    title: 'Foundation Of Web Technology',
    headerTitleStyle: {
      color: '#000',
      fontFamily: 'Roboto',
      marginLeft: 18
  ...
  componentDidMount() {
    firebase.database().ref("papers/semester-1/fwt/external/october/download").once('value', (data) => {
      this.setState({ octoberDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/fwt/external/april/download").once('value', (data) => {
      this.setState({ aprilDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/fwt/external/october/weightage").once('value', (data) => {
      this.setState({
    ...
    firebase.database().ref("papers/semester-1/fwt/external/april/weightage").once('value', (data) => {
      this.setState({
  ...
  OctoberdownloadPdf = () => {
    RNDM.download({
      ...
      title: 'FWT October External 2018 Pdf',
      description: 'downloading FWT October External 2018 paper'
    }).then(
  ...
  AprildownloadPdf = () => {
    RNDM.download({
      ...
      title: 'FWT April External 2018  Pdf',
      description: 'downloading FWT April External 2018 paper'
    }).then(
      ret => Vibration.vibrate(DURATION)
      ).catch(
      err => alert('Check internet connection or firebase client is temporarily down!', err.message)
  ...
  render() {
    const source1 = { uri: "bundle-assets://pdf/FWT_external.pdf" };
    const source2 = { uri: "bundle-assets://pdf/FWT_external.pdf" };
    return (
