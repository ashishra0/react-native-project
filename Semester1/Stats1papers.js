...
class Stats1papers extends Component {
  static navigationOptions = {
    title: 'IT Tools For Statistics 1',
    headerTitleStyle: {
      color: '#000',
      fontFamily: 'Roboto',
      marginLeft: 18
  ...
  componentDidMount() {
    firebase.database().ref("papers/semester-1/stats-1/external/october/download").once('value', (data) => {
      this.setState({ octoberDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/stats-1/external/april/download").once('value', (data) => {
      this.setState({ aprilDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/stats-1/external/october/weightage").once('value', (data) => {
      this.setState({
    ...
    firebase.database().ref("papers/semester-1/stats-1/external/april/weightage").once('value', (data) => {
      this.setState({
  ...
  OctoberdownloadPdf = () => {
    RNDM.download({
      ...
      title: 'Statistics 1 October External 2018 Pdf',
      description: 'downloading Statistics 1 October External 2018 paper'
    }).then(
  ...
  AprildownloadPdf = () => {
    RNDM.download({
      ...
      title: 'Statistics 1 April External 2018  Pdf',
      description: 'downloading Statistics 1 April External 2018 paper'
    }).then(
      ret => Vibration.vibrate(DURATION)
      ).catch(
      err => alert('Check internet connection or firebase client is temporarily down!', err.message)
  ...
  render() {
    const source1 = { uri: "bundle-assets://pdf/Stats1_external.pdf" };
    const source2 = { uri: "bundle-assets://pdf/Stats1_external.pdf" };
    return (
