...
import {
  ...
  TouchableHighlight,
  Modal,
  Vibration // changelog: import vibration
} from "react-native";
...
const DURATION = 1000; // changelog: set a duration for it to vibrate.
const { width: WIDTH } = Dimensions.get("window");
class BCpapers extends Component {
  ...
  componentDidMount() {
    firebase.database().ref("papers/semester-1/bc/external/october/download").once('value', (data) => {
      this.setState({ octoberDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/bc/external/april/download").once('value', (data) => {
      this.setState({ aprilDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/bc/external/october/weightage").once('value', (data) => {
      this.setState({
    ...
    firebase.database().ref("papers/semester-1/bc/external/april/weightage").once('value', (data) => {
      this.setState({
  ...
  OctoberdownloadPdf = () => {
    RNDM.download({
      ...
      title: 'BC October External 2018 Pdf',
      description: 'downloading BC October External 2018 paper'
    }).then(
  ...
  AprildownloadPdf = () => {
    RNDM.download({
      ...
      title: 'BC April External 2018  Pdf',
      description: 'downloading BC April External 2018 paper'
    }).then(
  ...
  render() {
    const source1 = { uri: "bundle-assets://pdf/BC_external.pdf" };
    const source2 = { uri: "bundle-assets://pdf/BC_external.pdf" };
    return (
