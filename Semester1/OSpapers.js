...
class OSpapers extends Component {
  ...
  componentDidMount() {
    firebase.database().ref("papers/semester-1/os/external/october/download").once('value', (data) => {
      this.setState({ octoberDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/os/external/april/download").once('value', (data) => {
      this.setState({ aprilDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/os/external/october/weightage").once('value', (data) => {
      this.setState({
    ...
    firebase.database().ref("papers/semester-1/os/external/april/weightage").once('value', (data) => {
      this.setState({
  ...
  OctoberdownloadPdf = () => {
    RNDM.download({
      ...
      title: 'OS October External 2018 Pdf',
      description: 'downloading OS October External 2018 paper'
    }).then(
  ...
  AprildownloadPdf = () => {
    RNDM.download({
      ...
      title: 'OS April External 2018  Pdf',
      description: 'downloading OS April External 2018 paper'
    }).then(
      ret => Vibration.vibrate(DURATION)
      ).catch(
      err => alert('Check internet connection or firebase client is temporarily down!', err.message)
  ...
  render() {
    const source1 = { uri: "bundle-assets://pdf/OS_external.pdf" };
    const source2 = { uri: "bundle-assets://pdf/OS_external.pdf" };
    return (
