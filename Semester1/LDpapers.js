...
class LDpapers extends Component {
  ...
  componentDidMount() {
    firebase.database().ref("papers/semester-1/ld/external/october/download").once('value', (data) => {
      this.setState({ octoberDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/ld/external/april/download").once('value', (data) => {
      this.setState({ aprilDownloadLink: data.toJSON().link })
    })
    firebase.database().ref("papers/semester-1/ld/external/october/weightage").once('value', (data) => {
      this.setState({
    ...
    firebase.database().ref("papers/semester-1/ld/external/april/weightage").once('value', (data) => {
      this.setState({
  ...
  OctoberdownloadPdf = () => {
    RNDM.download({
      ...
      title: 'LD October External 2018 Pdf',
      description: 'downloading LD October External 2018 paper'
    }).then(
  ...
  AprildownloadPdf = () => {
    RNDM.download({
      ...
      title: 'LD April External 2018  Pdf',
      description: 'downloading LD April External 2018 paper'
    }).then(
      ret => Vibration.vibrate(DURATION)
      ).catch(
      err => alert('Check internet connection or firebase client is temporarily down!', err.message)
  ...
  render() {
    const source1 = { uri: "bundle-assets://pdf/LD_external.pdf" };
    const source2 = { uri: "bundle-assets://pdf/LD_external.pdf" };
    return (
